	...
	bits 16		;x86 CPUs start in real mode which supports only 16-bit instructions
	org 0x7C00	;starting address for boot sector
	
	mov	ah, 0x01	
	mov	ch, 0x20	; ch = 20h disable cursor
	int 0x10		
	mov	ax, 0x0305	; ah = 03-->autorepeat operation
	mov	bx, 0x031F	; bh = amount of time to wait before starting autorepeat operation
					; bl = autorepeat rate
	int	0x16		; increase delay before keyboard repeat

game_loop:
	call clear_screen	; clear the screen
	push word [snake_pos] ; save snake head position for later
	mov	ah, 0x01	; check if key available in typeahead buffer(return ASCII/ScanCode in ax)
	int	0x16
...
move_done:
	...
	pop	ax 		; restore head position into ax for body shift
update_body:
	mov	bx, [si]	; get element of body into bx
	test bx, bx		; check if zero (not part of the body)
	jz	done_update	; if zero, done. Otherwise
	mov	[si], ax	; move the data from ax, into current position
	add	si, 2		; increment pointer by two bytes(next part of snake body)
...
check_collisions_self:
	...
	or	ax, ax		; check if body position is 0x0000 (we are done searching)
...
game_loop_continued:
	mov	cx, 0x0001	; Sleep for 0.17 seconds (cx:dx)
	mov	dx, 0xAFFF	; 0x000249F0 = 176127 micro seconds
	mov	ah, 0x86
	int	0x15		; Sleep
	jmp	game_loop	; loop
...
game_over:
	...
	mov dx, 0x0C21		; set pos to 0x0000
...
clear_screen:
	...
	mov	bh, 0x70	; Screen attribute of cleared area
	mov cx, 0x0000		; top left = (1,1)
	mov	dx, 0x1950	; bottom right = (22,52)
	int	0x10
...
move_cursor:
	mov	ah, 0x02	; move cursor to (dl- X, dh- Y)
	xor	bh, bh		; page 0	
	int 	0x10
	ret
